...
// forward declare for window file explorer
...
{
    // display current path (read only so the user wont break it)
    ImGui::InputText("Current path", path.data(), path.capacity(), ImGuiInputTextFlags_ReadOnly);

    // flag to check loading model
    ...
    if (ImGui::Button("Browse File..."))
    {
        ...
        // only update if the user pick something (otherwise do nothing)
        if (!newPath.empty())
        {
            // windows return path like C:\users\...
            // replace \ with /
            std::replace(newPath.begin(), newPath.end(), '\\', '/');
            path = newPath;     // update the path variable in main

            shouldLoad = true;
        }
    }

    // Manual loading button (keep for reloading)
    ...
    if (shouldLoad)
    {
        ...
        {
            ImGui::Text("Failed to load model: %s", path.c_str());
    ...
    ImGui::SliderFloat("##scaleSlider", &scale, 0.01f, 1.0f, "%.3f");
    ImGui::SameLine();
    ImGui::SetNextItemWidth(80.0f);  // Make input box narrower
    ImGui::InputFloat("Manual", &scale, 0.01f, 0.01f, "%.4f");
