...
int NB_STEPS{ 50 };     // ray marching steps
bool volumetricToggle{ true };
...
int main()
{
    ...
    // Volumetric lighting
    Shader volumetricLighting("resources/shader/volumetric/volumetric.vs", "resources/shader/volumetric/volumetric.fs");
    // load models
   ...
   // render all 6 faces in 1 pass using geometry shader (dont have to loop 6 times)
    ...
    // volumetric lighting
    volumetricLighting.use();
    volumetricLighting.setInt("gPosition", 0);
    volumetricLighting.setInt("shadowMapDir", 1);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glCullFace(GL_BACK);           // Render shadows exactly like the eye sees the object
        glEnable(GL_POLYGON_OFFSET_FILL);
        glPolygonOffset(1.1f, 4.0f);   // Push geometry back slightly in the depth buffer
        ...
        if (volumetricToggle)
        {
            ...
            // since we want to add fog on top of our existing scene color
            glEnable(GL_BLEND);
            glBlendFunc(GL_ONE, GL_ONE);

            volumetricLighting.use();
            volumetricLighting.setInt("NB_STEPS", NB_STEPS);
            glActiveTexture(GL_TEXTURE0);
            glBindTexture(GL_TEXTURE_2D, gPosition);
            glActiveTexture(GL_TEXTURE1);
            glBindTexture(GL_TEXTURE_2D, depthMap); // directional shadow map

            volumetricLighting.setMat4("lightSpaceMatrix", lightSpaceMatrix);
            volumetricLighting.setVec3("viewPos", camera.Position);
            volumetricLighting.setVec3("lightPos", lightPos);
            volumetricLighting.setVec3("lightColor", dirLightData.diffuse);
            renderQuad();
        ...
        if (showImGuiWindow)
        {
            ...
            if (deltaTime * 1000.0f > maxFrameTime)
                maxFrameTime = deltaTime * 1000.0f;

            frameCounter++;
            if (frameCounter > 100)
            {
                // refresh every 100 frames 
                ImGui::Text("Max frame time (Stutter): %.2f ms", maxFrameTime);
                maxFrameTime = 0.0f;
                frameCounter = 0;
            }
            ...
            if (ImGui::Checkbox("Use Blinn-Phong lighting", &blinn));

            ImGui::Text("Volumetric lighting: ");
            if (ImGui::Checkbox("Turn volumetric lighting on/off", &volumetricToggle));

            ImGui::Text("Debug quad (For deferred shading):");
            if (ImGui::Checkbox("Turn debug quads on/off", &debugButton));
            ...
            ImGui::SliderInt("Ray marching steps", &NB_STEPS, 1, 100);
...
void directionalLightChange()
{
    if (ImGui::TreeNode("Directional light"))
    {
        ...
        static float azimuth = 90.0f;   // Rotation around the Y-axis (0-360)
        static float elevation = 45.0f; // Height from the horizon (-90 to 90)

        if (ImGui::TreeNode("Orientation (Sun Position)"))
        {
            // The Problem: Sliders for X/Y/Z (-1 to 1) are unintuitive for direction. 
            // The Intent: Use angles (degrees) which humans understand naturally for "Day/Night" cycles.
            bool changed = false;
            changed |= ImGui::SliderFloat("Azimuth (Rotation)", &azimuth, 0.0f, 360.0f);
            changed |= ImGui::SliderFloat("Elevation (Height)", &elevation, -90.0f, 90.0f);

            // The Mechanism: Convert Spherical coordinates (Angles) to Cartesian (Vector).
            // We calculate the position of the sun, then negate it because Light Direction points FROM sun TO fragment.
            if (changed) {
                float azRad = glm::radians(azimuth);
                float elRad = glm::radians(elevation);

                // Calculate where the "Sun" is
                float x = sin(azRad) * cos(elRad);
                float y = sin(elRad);
                float z = cos(azRad) * cos(elRad);

                // The direction of the light ray is opposite to the sun's position
                dirLightData.direction = glm::normalize(-glm::vec3(x, y, z));
            }

            // Visual feedback of the actual vector being sent to the shader
            ImGui::Text("Resulting Vec3: (%.2f, %.2f, %.2f)",
                dirLightData.direction.x, dirLightData.direction.y, dirLightData.direction.z);

            ImGui::TreePop();
        }

        if (ImGui::TreeNode("Lighting Properties"))
        {
